...
class TimeAwareNodeModel(nn.Module):
    ...
    """
    def __init__(self, flow_out_mlp, node_mlp, node_agg_fn):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        mess_out = self.flow_out_mlp(mess_in)
        flow = self.node_agg_fn(mess_out, row, x.size(0))
...
class MOTMPNet(nn.Module):
    ...
    def _build_core_MPNet(self, model_params, encoder_feats_dict):
        ...
        node_mlp = MLP(input_dim = encoder_feats_dict['node_out_dim'],
