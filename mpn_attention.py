...
class TimeAwareNodeModel(nn.Module):
    ...
    """
    def __init__(self, flow_out_mlp, node_mlp, node_agg_fn, attention_head_num):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        row, col = edge_index
        mess_in = torch.cat([x[row], edge_attr], dim=1)
        xx = torch.cat([x[row],x[col]],dim=1)
        ...
        for i in range(attention_head_num):
            mess_out.append(self.flow_out_mlp[i](mess_in))
            atnet.append(nn.Conv1d(2*x.shape[1],1,1, stride=1,bias=False).cuda())
            e.append(atnet[i](xx).view(-1))
            a.append(torch_scatter.composite.scatter_softmax(e[i],row, dim=0, eps=1e-12))
            flow.append(self.node_agg_fn(a[i], mess_out[i], row, x.size(0)))
        flow1 = torch.cat(flow,dim=1)
        return self.node_mlp(flow1)
...
class MOTMPNet(nn.Module):
    ...
    def _build_core_MPNet(self, model_params, encoder_feats_dict):
        ...
        node_mlp = MLP(input_dim = encoder_feats_dict['node_out_dim']*attention_head_num,
