    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        if self.time_aware:
            ...
            if self.use_attention:
                ...
                flow_out = torch.mul(flow_out.transpose(0, 2), a_out.transpose(0, 1)).transpose(0, 1).transpose(1,
                                                                                                                2).reshape(
                    flow_out_row.shape[0], -1)
                # [d=32,M1,k] [M1,k] -> [d=32,M1,k] -> [M1,32,k] -> [M1,k,32] -> [M1,32k]
                flow_in = torch.mul(flow_in.transpose(0, 2), a_in.transpose(0, 1)).transpose(0, 1).transpose(1,
                                                                                                             2).reshape(
                    flow_in_row.shape[0], -1)
                # [d=32,M2,k] [M2,k] -> [d=32,M2,k] -> [M2,32,k] -> [M2,k,32] -> [M2,32k]
                flow_out = self.node_agg_fn(flow_out, flow_out_row, x.size(0))  # [M1,32k] -> [N,32k]
                flow_in = self.node_agg_fn(flow_in, flow_in_row,
                                           x.size(0))  # [M,32k] -> [N,32k]          # [M2,32k] -> [N,32k]
                ...
                flow_out = self.node_agg_fn(torch.squeeze(flow_out, 0), flow_out_row,
                                            x.size(0))  # [1,M1,d=32] -> [M1,d=32] -> [N,32]
                flow_in = self.node_agg_fn(torch.squeeze(flow_in, 0), flow_in_row,
                                           x.size(0))  # [1,M2,d=32] -> [M2,d=32] -> [N,32]
            ...
            if self.use_attention:
                ...
                flow = torch.mul(flow.transpose(0, 2), a.transpose(0, 1)).transpose(0, 1).transpose(1, 2).reshape(
                    row.shape[0], -1)
        ...
        if self.use_attention:
            ...
            return self.node_mlp(flow)  # [N,32]
