...
class TimeAwareNodeModel(nn.Module):
    ...
    """
    def __init__(self, flow_in_mlp, flow_out_mlp, node_mlp, node_agg_fn, attention_head_num):
    ...
    def forward(self, x, edge_index, edge_attr):
        ...
        mess_out_input = torch.cat([x[flow_out_row], edge_attr[flow_out_mask]], dim=1)
        xx_out = torch.cat([x[flow_out_row],x[flow_out_col]],dim=1)
        xx_out = torch.unsqueeze(xx_out,2)
        mess_out = []
        atnet_out = []
        e_out = []
        a_out = []
        ...
        for i in range(attention_head_num):
            mess_out.append(self.flow_out_mlp[i](mess_out_input))
            atnet_out.append(nn.Conv1d(2*x.shape[1],1,1, stride=1,bias=False).cuda())
            e_out.append(atnet_out[i](xx_out).view(-1))
            a_out.append(torch_scatter.composite.scatter_softmax(e_out[i],flow_out_row, dim=0, eps=1e-12))
            flow_out.append(self.node_agg_fn(a_out[i], mess_out[i], flow_out_row, x.size(0)))
        flow1_out = torch.cat(flow_out,dim=1)

        flow_in_mask = row > col
        flow_in_row, flow_in_col = row[flow_in_mask], col[flow_in_mask]
        mess_in_input = torch.cat([x[flow_in_row], edge_attr[flow_in_mask]], dim=1)
        xx_in = torch.cat([x[flow_in_row],x[flow_in_col]],dim=1)
        xx_in = torch.unsqueeze(xx_in,2)
        mess_in = []
        atnet_in = []
        e_in = []
        a_in = []
        flow_in = []
        for i in range(attention_head_num):
            mess_in.append(self.flow_in_mlp[i](mess_in_input))
            atnet_in.append(nn.Conv1d(2*x.shape[1],1,1, stride=1,bias=False).cuda())
            e_in.append(atnet_in[i](xx_in).view(-1))
            a_in.append(torch_scatter.composite.scatter_softmax(e_in[i],flow_in_row, dim=0, eps=1e-12))
            flow_in.append(self.node_agg_fn(a_in[i], mess_in[i], flow_in_row, x.size(0)))
        flow1_in = torch.cat(flow_in,dim=1)
        flow = torch.cat((flow1_in, flow1_out), dim=1)
...
class MOTMPNet(nn.Module):
    ...
    def _build_core_MPNet(self, model_params, encoder_feats_dict):
        ...
        attention_head_num = model_params['attention_head_num']

        assert node_agg_fn.lower() in ('mean', 'max', 'sum','attention'), "node_agg_fn can only be 'max', 'mean', 'sum', or 'attention'."

        if node_agg_fn == 'mean':
            node_agg_fn = lambda a,out, row, x_size: scatter_mean(out, row, dim=0, dim_size=x_size)

        elif node_agg_fn == 'max':
            node_agg_fn = lambda a,out, row, x_size: scatter_max(out, row, dim=0, dim_size=x_size)[0]

        elif node_agg_fn == 'sum':
            node_agg_fn = lambda a,out, row, x_size: scatter_add(out, row, dim=0, dim_size=x_size)
        
        elif node_agg_fn == 'attention':
            node_agg_fn =  lambda a, out, row, x_size: scatter_add(torch.mul(out.T,a).T,row,dim=0, dim_size=x_size)
        ...
        node_mlp = MLP(input_dim = encoder_feats_dict['node_out_dim']*attention_head_num*2,
